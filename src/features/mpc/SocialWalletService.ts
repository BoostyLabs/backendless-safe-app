...
import { SecurityQuestionRecovery } from '@/features/mpc/recovery/SecurityQuestionRecovery';
import { DeviceShareRecovery } from '@/features/mpc/recovery/DeviceShareRecovery';
...
class SocialWalletService implements ISocialWalletService {
  ...
  isMFAEnabled() {
    ...
    return !Object.values(shareDescriptions).some(value =>
  ...
  async enableMFA(oldPassword: string | undefined, newPassword: string): Promise<void> {
    try {
      ...
      const securityQuestionFactor =
        await this.securityQuestionRecovery.recoverWithPassword(newPassword);
